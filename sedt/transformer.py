...
class TransformerDecoderLayerParallelAttn(nn.Module):

    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, num_class=10):
        super().__init__()
        self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        multihead_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.multihead_parallel_attn = _get_clones(multihead_attn, num_class)
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt = self.norm1(tgt)
        tgt2 = []
        for i, multihead in enumerate(self.multihead_parallel_attn):
            tgt2.append(
                multihead(query=self.with_pos_embed(tgt[i*sub_num_q:(i+1)*sub_num_q, :, :].unsqueeze(dim=0),
                                                    query_pos[i*sub_num_q:(i+1)*sub_num_q, :, :].unsqueeze(dim=0)),
                          key=self.with_pos_embed(memory, pos),
                          value=memory, attn_mask=memory_mask,
                          key_padding_mask=memory_key_padding_mask)[0])
        tgt2 = torch.cat(tgt2, dim=0)
        tgt = tgt + self.dropout2(tgt2)
        tgt = self.norm2(tgt)
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt))))
        tgt = tgt + self.dropout3(tgt2)
    ...
    def forward(self, tgt, memory,
        ...
        if self.normalize_before:
            return self.forward_pre(tgt, memory, tgt_mask, memory_mask,
                                    tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos, sub_num_q)
        return self.forward_post(tgt, memory, tgt_mask, memory_mask,
                                 tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos, sub_num_q)
