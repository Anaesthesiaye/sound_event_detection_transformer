...
def get_dfs(desed_dataset, dataname, unlabel_data=False):
    if dataname == 'urbansed':
        ...
        # train_df = pd.read_csv("/home/yzr/data/yzr/urban_SED_data/feature/metadata/train_filtered.tsv", sep='\t')
        # valid_df = pd.read_csv("/home/yzr/data/yzr/urban_SED_data/feature/metadata/test_filtered.tsv", sep='\t')
        # eval_df = pd.read_csv("/home/yzr/data/yzr/urban_SED_data/feature/metadata/eval_filtered.tsv", sep='\t')
        ...
        # validation_df = pd.read_csv("/home/yzr/data/yzr/dcase2020/dataset/features/sr16000_win1024_hop323_mels64_nolog/metadata/audio/validation/validation.tsv", sep='\t')
        ...
        if unlabel_data:
            ...
            data_dfs['dcase2018_task5'] = dcase2018_task5
...
if __name__ == '__main__':
    ...
    # set certain parameters for fine_tune
    # if f_args.fine_tune:
    #     f_args.lr = 1e-5
    #     f_args.lr_backbone = 1e-5
    #     f_args.lr_drop = 100
    #     f_args.epochs = 200
    if f_args.self_sup:
    ...
    if f_args.self_sup:
        ...
        scaler_path = os.path.join(store_dir, f_args.dataname + "_cp.json")
    ...
    if f_args.pretrain:
        ...
        model_fname = "/home/yzr/data/yzr/python_workspace/sed_transformer/stored_data/audio_tag/model/resnet50_max_pre:False_dcase_backbone_audio_tagging"
    ...
    if not f_args.self_sup:
        if cfg.save_best or f_args.eval:
            for at_m in fusion_strategy:
                # model_fname = os.path.join(saved_model_dir, f"{f_args.info}_{at_m}_best")
                model_fname = "/home/yzr/data/yzr/python_workspace/sed_transformer/stored_data/dcase_with_synthetic_unlabel_with_weak/model/pretrained_weakloss_0.25_weakploss_only_weak_1.0_enc_6_pooling_None_[1, 2, 3]_pretrained_self_sedt_big_data_enc_6_loss_159_0.5_1_best"
                if torch.cuda.is_available():
                    state = torch.load(model_fname)
                else:
                    state = torch.load(model_fname, map_location=torch.device('cpu'))
